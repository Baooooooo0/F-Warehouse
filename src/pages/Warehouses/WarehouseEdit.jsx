...
const WarehouseEdit = () => {
    ...
    const fetchWarehouseDetails = async () => {
        try {
            ...
            const response = await warehouseAPI.getAll({ search: '' });

            if (response.code === 'success' && response.data) {
                const warehouse = response.data.find(w => w.id === Number(id));
                if (warehouse) {
                    setFormData({
                        name: warehouse.name || '',
                        address: warehouse.address || '',
                        image: null,
                        currentImage: warehouse.image || '',
                    });
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await warehouseAPI.update(id, submitData);

            if (response.code === 'success') {
                toast.success('Cập nhật kho hàng thành công!');
                navigate('/warehouses');
            } else {
                toast.error(response.message || 'Không thể cập nhật kho hàng');
            }
        } catch (error) {
    ...
    if (fetchingWarehouse) {
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
                    <p className="text-slate-600">Đang tải thông tin kho hàng...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="sticky top-0 z-10 bg-white border-b border-slate-200 px-8 py-4 mt-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900">Chỉnh sửa kho hàng</h1>
                            <p className="text-sm text-slate-500 mt-0.5">Cập nhật thông tin kho hàng</p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                            <div className="p-6 border-b border-slate-200">
                                ...
                                <p className="text-sm text-slate-500 mt-1">Chỉnh sửa thông tin kho hàng</p>
                            </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                            <div className="p-6 border-b border-slate-200">
                                ...
                                <p className="text-sm text-slate-500 mt-1">Thay đổi hình ảnh kho hàng (tùy chọn)</p>
                            </div>
                            <div className="p-6">
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium text-slate-900">
                                                    {formData.image ? 'Thay đổi hình ảnh' : 'Nhấn để tải lên hình ảnh mới'}
                                                </p>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex items-center justify-between text-sm">
                                        ...
                                        <span className="font-medium text-slate-900">
                                            {formData.image ? 'Đã chọn mới' : (formData.currentImage ? 'Giữ nguyên' : 'Chưa có')}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="pt-4 border-t border-slate-200">
                                    <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-lg border border-blue-100">
                                        ...
                                        <p className="text-xs text-blue-700 leading-relaxed">
                                            Nếu bạn không chọn hình ảnh mới, hình ảnh hiện tại sẽ được giữ nguyên.
                                        </p>
                                    </div>
                                </div>
                            </div>
